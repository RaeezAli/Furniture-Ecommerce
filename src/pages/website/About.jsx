...
export default function About() {
  // Define articles array above your component or inside it
  const articles = [
    {
      img: img1,
    ...
    // Add more articles here and pagination will auto-adjust
  ];
  ...
  const getBlogPageNumbers = () => {
    if (totalBlogPages <= 3)
      return Array.from({ length: totalBlogPages }, (_, i) => i + 1);
    if (blogPage === 1) return [1, 2, 3];
    if (blogPage === totalBlogPages)
      return [totalBlogPages - 2, totalBlogPages - 1, totalBlogPages];
    return [blogPage - 1, blogPage, blogPage + 1];
  };
    ...
    <main className="w-full pt-16">
      <div className="bg-[#F9F1E7] relative h-[316px] flex flex-col items-center justify-center bg-[url('/src/Images/Common/bg.jpg')] bg-cover bg-center">
        ...
        <div className="relative z-10 text-center">
          <h1 className="text-[48px] font-bold text-black">Blog</h1>
          <div className="flex items-center gap-2 justify-center text-[16px]">
            <span className="font-bold">Home</span>
            <span className="font-bold">{">"}</span>
            <span className="font-medium text-[#333333]">Blog</span>
          </div>
        </div>
      </div>

      <section className="w-full mt-8 md:px-16 md:pb-8">
        ...
        <div className="flex flex-col-reverse md:flex-row justify-between gap-6 md:gap-8 lg:gap-12">
          ...
          <div className="w-full md:w-2/3 flex flex-col items-center md:items-start">
            {paginatedArticles.map((article, idx) => (
              <article key={idx} className="w-full mb-12">
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                  ...
                  {blogPage > 1 && (
                    <button
                      ...
                      className="bg-[#F9F1E7] text-gray-800 w-16 h-10 sm:w-[98px] sm:h-[60px] rounded-xl sm:rounded-2xl flex justify-center items-center font-medium hover:bg-[#B88E2F] hover:text-white transition-all"
                    >
                      Prev
                    </button>
                  )}
                  ...
                  {getBlogPageNumbers().map((page) => (
                    <button
                      ...
                      className={`w-10 h-10 sm:w-[60px] sm:h-[60px] rounded-xl sm:rounded-2xl flex justify-center items-center font-medium transition-all
              ${
                blogPage === page
                  ? "bg-[#B88E2F] text-white"
                  : "bg-[#F9F1E7] text-gray-800 hover:bg-[#B88E2F] hover:text-white"
              }`}
                    >
                      {page}
                    </button>
